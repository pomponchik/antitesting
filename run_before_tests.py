...
date_before, date_now, date_tomorrow = (datetime.datetime.now() - datetime.timedelta(day=-1)).date(), datetime.datetime.now().date(), (datetime.datetime.now() - datetime.timedelta(day=1)).date()
...
with open(os.path.join("examples", "forbidding_file_example.txt"), "r", encoding="utf-8") as file:
    content = read().format(date_before, date_now, date_tomorrow)
